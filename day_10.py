...
def check_signal_strength_sum(cycle, x_register, signal_strength_sum):
    if cycle % SCREEN_WIDTH == 20:
        signal_strength = cycle * x_register
        signal_strength_sum += signal_strength
        print(f'Cycle: {cycle} Signal Strength: {signal_strength} Signal Strength Sum: {signal_strength_sum}')

    return signal_strength_sum



def get_sprite_pos(x_register):
    sprite_pos = list(range(x_register - 1, x_register + 2))
    return sprite_pos



def draw_pixel(cycle, x_register, line_buffer):
        pixel_pos = (cycle - 1) % SCREEN_WIDTH
        sprite_pos = get_sprite_pos(x_register)
...
def process_data(data):
    ...
    for line in data:
        cycle += 1
        signal_strength_sum = check_signal_strength_sum(cycle, x_register, signal_strength_sum)

        line_buffer = draw_pixel(cycle, x_register, line_buffer)
        ...
        if cmd == 'addx':
            ...
            x_register += int(line_split[1])

    
    for i in list(range(len(line_buffer)))[0::SCREEN_WIDTH]:
        print(line_buffer[i:i+SCREEN_WIDTH-1])
...
def run():
    input_data_sample = read_input('./day_10_input_sample.txt')
    input_data = read_input('./day_10_input.txt')

    print('DAY 10')

    # Part 1 Answer
    answer_1_sample = process_data(input_data_sample)
    print(f'Answer 1 Sample: {answer_1_sample}') # 13140
    answer_1 = process_data(input_data)
    print(f'Find the signal strength during the 20th, 60th, 100th, 140th, 180th, and 220th cycles. What is the sum of these six signal strengths? : {answer_1}') # 15680

    # Part 2 Answer
    print(f"Render the image given by your program. What eight capital letters appear on your CRT? : ZFBFHGUP")
