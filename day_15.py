...
def read_input(file_path):
    ...
    sensor_closest_beacon_lookup = dict()

    for line in lines:
        matches = re.findall(r'-?\d+', line)
        sensor_pos = (int(matches[0]), int(matches[1]))
        closest_beacon_pos = (int(matches[2]), int(matches[3]))
        sensor_closest_beacon_lookup[sensor_pos] = closest_beacon_pos

    return sensor_closest_beacon_lookup



def get_taxicab_distance(pos_1, pos_2):
    taxicab_distance = abs(pos_1[0] - pos_2[0]) + abs(pos_1[1] - pos_2[1])
    return taxicab_distance



def process_data(sensor_closest_beacon_lookup):
