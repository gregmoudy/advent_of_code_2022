...
# it is holding one at a time and in the order listed. Monkey 0 goes first, then monkey 1, and so on until each monkey has had one turn. 
...
KEY_DIVISIBLE_BY = 'divisible_by'
KEY_THROW_TO_TRUE = 'throw_to_true'
KEY_THROW_TO_FALSE = 'throw_to_false'
KEY_INSPECTION_COUNT = 'inspection_count'
...
def read_input(file_path):
    ...
    for i in list(range(0, len(lines), 7)):
        monkey_data = dict()
        monkey_data[KEY_ITEMS] = [int(x) for x in re.findall(r"\d+", lines[i+1])]
        monkey_data[KEY_OPERATION] = re.findall(r"old [*+] .*$", lines[i+2])[0]
        monkey_data[KEY_DIVISIBLE_BY] = int(re.findall(r"\d+", lines[i+3])[0])
        monkey_data[KEY_THROW_TO_TRUE] = int(re.findall(r"\d+", lines[i+4])[0])
        monkey_data[KEY_THROW_TO_FALSE] = int(re.findall(r"\d+", lines[i+5])[0])
        monkey_data[KEY_INSPECTION_COUNT] = 0
        monkey_datas[monkey_num] = monkey_data
...
def process_data(monkey_datas, rounds = 20):
    big_mod = 1
    for monkey_data in monkey_datas.values():
         big_mod *= monkey_data[KEY_DIVISIBLE_BY]

    for _round in range(0, rounds): # Number of rounds.
        for monkey_num in range(len(monkey_datas)):
            monkey_data = monkey_datas[monkey_num]
            items = monkey_data[KEY_ITEMS]
            for i, old in enumerate(list(items)):
                monkey_data[KEY_INSPECTION_COUNT] += 1
                new = eval(monkey_data[KEY_OPERATION])

                if rounds <= 20:
                    new = new // 3 # Monkey get's bored with item and my worry level drops.

                else:
                    new = new % big_mod # Part 2

                if new % monkey_data[KEY_DIVISIBLE_BY] == 0:
                    throw_to_monkey = monkey_data[KEY_THROW_TO_TRUE]
                else:
                    throw_to_monkey = monkey_data[KEY_THROW_TO_FALSE]

                monkey_datas[throw_to_monkey][KEY_ITEMS].append(new)
                items.pop(0)

    inspection_counts = [x[KEY_INSPECTION_COUNT] for x in monkey_datas.values()]
...
def run():
    input_data_sample = read_input('./day_11_input_sample.txt')
    input_data = read_input('./day_11_input.txt')
    ...
    #print(f'Answer 2 Sample: {answer_2_sample}') # 2713310158
    answer_2 = process_data(input_data, rounds = 10000)
    print(f'Starting again from the initial state in your puzzle input, what is the level of monkey business after 10000 rounds? : {answer_2}') # 25590400731
